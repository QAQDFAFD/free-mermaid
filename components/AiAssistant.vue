  ...
  <div class="relative">
    ...
    >
      ...
      >
        ...
        <!-- 设置面板 -->
        <div v-show="showSettings" class="p-3 md:p-4 bg-gray-50 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 flex-1 overflow-y-auto">
          <!-- Key 来源选择 -->
          <div class="mb-4">
            ...
            <div class="flex gap-2">
              ...
              >
                ...
                <div class="text-xs opacity-70">{{ hasDefaultKey ? t('ai.freeKeyDesc') : t('ai.freeKeyUnavailable') }}</div>
              </button>
              <button
                @click="config.keySource = 'custom'"
                class="flex-1 px-3 py-2 text-sm rounded-lg border-2 transition-all duration-200"
                :class="config.keySource === 'custom' 
                  ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300' 
                  : 'border-gray-200 dark:border-gray-600 hover:border-gray-300 dark:hover:border-gray-500 text-gray-600 dark:text-gray-400'"
              >
                <div class="font-medium flex items-center justify-center gap-1">
          ...
          <!-- 免费额度提示（仅在选择默认时显示） -->
          <div v-show="config.keySource === 'default'" class="p-3 bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-800 rounded-lg">
        ...
        <div v-show="!showSettings" class="p-3 md:p-4 flex-1 overflow-y-auto">
          ...
          <div v-if="mode === 'optimize'" class="space-y-3">
            ...
            <div class="p-3 bg-gray-50 dark:bg-gray-900 rounded-lg max-h-32 overflow-auto">
              <pre class="text-xs text-gray-700 dark:text-gray-300 whitespace-pre-wrap font-mono">{{ currentCode || t('ai.noCode') }}</pre>
            </div>
          </div>
          ...
          <div v-if="streamOutput" class="mt-4">
            ...
            <div class="p-3 bg-gray-900 rounded-lg max-h-48 overflow-auto">
              <pre class="text-xs text-green-400 whitespace-pre-wrap font-mono">{{ streamOutput }}<span v-if="isLoading" class="animate-pulse">▌</span></pre>
            </div>
          </div>

          <!-- 错误提示 -->
          <div v-if="error" class="mt-3 p-3 bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded-lg">
          ...
          <div class="mt-4 flex gap-2">
            ...
            <button
              @click="handleAction"
              :disabled="isLoading || !hasValidKey || (mode === 'generate' && !topic.trim()) || (mode === 'optimize' && !currentCode.trim())"
            ...
            >
              ...
              <span>{{ isLoading ? t('ai.generating') : (streamOutput ? t('ai.regenerate') : (mode === 'optimize' ? t('ai.optimize') : t('ai.generate'))) }}</span>
            </button>
          </div>

          <!-- 未配置 API Key 提示 -->
          <div v-if="!hasValidKey" class="mt-3 p-3 bg-amber-50 dark:bg-amber-900/30 border border-amber-200 dark:border-amber-800 rounded-lg">
