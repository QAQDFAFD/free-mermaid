          ...
          if (svgElement) {
            ...
            if (currentDarkMode) {
              ...
              textElements.forEach(textElement => {
                ...
                if (currentFill && ['#000', '#000000', 'black', 'rgb(0,0,0)'].includes(currentFill.toLowerCase())) {
                  textElement.setAttribute('fill', '#e5e7eb') // 使用灰色-200作为默认亮色
                }
              })

              // 对所有路径和形状元素应用适当的深色模式样式
              const pathElements = svgElement.querySelectorAll('path, rect, circle, ellipse, line, polygon, polyline')
              pathElements.forEach(element => {
                // 如果是描边且为黑色，改为亮色
                const stroke = element.getAttribute('stroke')
                if (stroke && ['#000', '#000000', 'black', 'rgb(0,0,0)'].includes(stroke.toLowerCase())) {
  ...
  watch(
    ...
    async newCode => {
      ...
      if (newCode === undefined || newCode === null || newCode === '') {
        ...
        if (diagramRef.value) {
          // 清空内容并显示占位符，使用适当缩放补偿90%缩放
          diagramRef.value.innerHTML = `
						<div class="placeholder-wrapper" style="transform: scale(1.11); transform-origin: center center;">
							<div class="flex items-center justify-center h-full w-full text-gray-400 dark:text-gray-500 text-xl">
								${t('preview.placeholder')}
							</div>
						</div>
